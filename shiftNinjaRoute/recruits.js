...
const validateRecruit = require('../Services/validateUserDataInput.js');
const forwardingService = require('../Services/recruitsForwardingService.js');
...
const recruitModel = require('../domainModels/recruitsModel.js');
...
rOut.get('/viewRecruitByID/:recID', function (req, res, next) {
    ...
    recruitModel.findOne({ RecruitRef: req.params.RecruitRef }).then(function (Recruit) {
        var productsList = [];
        var count = 0;
...
rOut.get('/displayRecruits/:custoRef', function (req, res, next) {
    recruitModel.find({ custoRef: req.params.custoRef }).then(function (Recruit) {
        res.render('viewRecruit', { RecruitList: Recruit });
    }).catch(next);
});
